...
var configItems = {
    backgroundColor : 0x943855,
    groundColor : 0xffd692,
    obstacleColor : 0xffd692,
    playerColor : 0xa0cc78,
    noOfGround : 3,
...
export default class Level2 extends Phaser.Scene{
    ...
    initUI(){
         this.add.text(configItems.width/2,15,"Level 2",{'fontSize':'24px','fill': '#fff'}).setOrigin(.5,.5);
    ...
    createBlocks(){
        ...
        let g1 = this.physics.add.staticImage(configItems.width/3,window.innerHeight * .3,'tile')
        .setOrigin(0.5,1)
        .setScale(1,2).refreshBody();
        g1.depth = -10;
        

        let g2 = this.physics.add.staticImage(configItems.width * 2/3,window.innerHeight * .3,'tile')
        .setOrigin(0.5,1)
        .setScale(1,2).refreshBody();
        g2.depth = -10;
        ...
        let g4 = this.physics.add.staticImage(configItems.width/3,window.innerHeight * .9,'tile')
        .setOrigin(0.5,1)
        .setScale(1,2).refreshBody();
        g4.depth = -10;

        let g5 = this.physics.add.staticImage(configItems.width * 2/3,window.innerHeight * .9,'tile')
        .setOrigin(0.5,1)
        .setScale(1,1.5).refreshBody();
        g5.depth = -10;

        g1.tint = g2.tint = g3.tint = g4.tint = g5.tint = 0xffd692;
