...
var configItems = {
    backgroundColor : 0x49beb7,
    groundColor : 0x200f21,
    obstacleColor : 0xdc5353,
    playerColor : 0xed1250,
    noOfGround : 3,
...
export default class Level3 extends Phaser.Scene{
    ...
    initUI(){
         this.add.text(configItems.width/2,15,"Level 3",{'fontSize':'24px','fill': '#fff'}).setOrigin(.5,.5);
    ...
    createBlocks(){
        ...
        let g1 = this.physics.add.staticImage(configItems.width/4,window.innerHeight * .3,'tile')
        .setOrigin(0.5,1)
        .setScale(1,2).refreshBody();
        g1.depth = -10;
        

        let g2 = this.physics.add.staticImage(configItems.width * 2/4,window.innerHeight * .3,'tile')
        .setOrigin(0.5,1)
        .setScale(1,2).refreshBody();
        g2.depth = -10;

        let g21 = this.physics.add.staticImage(configItems.width * 3/4,window.innerHeight * .3,'tile')
        .setOrigin(0.5,1)
        .setScale(1,2).refreshBody();
        g21.depth = -10;

        let g3 = this.physics.add.staticImage(configItems.width /3,window.innerHeight * .6,'tile')
        .setOrigin(0.5,1)
        .setScale(4,1).refreshBody();
        g3.depth = -10;

        let g31 = this.physics.add.staticImage(configItems.width * 2 /3,window.innerHeight * .6,'tile')
        .setOrigin(0.5,1)
        .setScale(4,1).refreshBody();

        let g4 = this.physics.add.staticImage(configItems.width/2,window.innerHeight * .6,'tile')
        .setOrigin(0.5,1)
        .setScale(1,-2).refreshBody();
        g4.depth = -10;


        let g5 = this.physics.add.staticImage(configItems.width /2,window.innerHeight * .9,'tile')
        .setOrigin(0.5,1)
        .setScale(1,1.76).refreshBody();

        g1.tint = g2.tint = g3.tint = g4.tint = g5.tint = g21.tint = g31.tint = configItems.obstacleColor;
